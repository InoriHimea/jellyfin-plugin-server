...
// CompatConfig sets the administrator's fallback Jellyfin version. It is used
// when a manifest request has neither an explicit /manifest/{version} path
// nor a recognizable Jellyfin User-Agent, and as the global runtime policy
// for background package downloads and cache validation. Requests that
// identify their Jellyfin version are filtered per request instead.
